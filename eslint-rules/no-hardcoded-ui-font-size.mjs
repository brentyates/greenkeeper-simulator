...
export default {
  meta: {
    type: 'problem',
    docs: {
      description: 'Disallow hardcoded button fontSize literals in UI files',
      category: 'Best Practices',
    },
    schema: [],
    messages: {
      noHardcodedFontSize: 'Do not hardcode button fontSize in UI files. Use UI_THEME typography tokens or shared helper defaults.',
    },
  },
  create(context) {
    ...
    return {
      AssignmentExpression(node) {
        if (
          ...
          node.left.property.name === 'fontSize' &&
          node.right?.type === 'Literal' &&
          typeof node.right.value === 'number' &&
          isButtonLikeTarget(node.left)
        ) {
          context.report({ node, messageId: 'noHardcodedFontSize' });
        }
      },
    };
  },
